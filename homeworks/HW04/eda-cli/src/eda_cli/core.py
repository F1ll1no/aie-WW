...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    def const_col(summary: DatasetSummary) -> bool:
        for col in summary.columns: 
            if col.unique == 1 or col.unique == 0:
                return True
        return False

    def id_dup(summary: DatasetSummary) -> bool:
        for col in summary.columns:  
            if col.name == "user_id" and col.unique == col.non_null:
                return True
        return False

    flags["has_constant_columns"] = const_col(summary)
    flags["has_suspicious_id_duplicates"] = id_dup(summary)
