...
class QualityFlagsResponse(BaseModel):
    ...
    too_many_columns: bool = Field(
        ...,
        description="Слишком много колонок",
    )
    ...
    has_suspicious_id_duplicates: bool = Field(
        ...,
        description="Есть дубликаты id пользователей",
    )
    has_constant_columns: bool = Field(
        ...,
        description="Есть колонки с постоянным значением",
    )
    quality_score: float = Field(
        ...,
        ge=0.0,
...
@app.post(
    ...
    tags=["quality"],
    summary="Вывод всех реализованных эвристик",
)
async def quality_flags_from_csv(file: UploadFile = File(...)) -> QualityFlagsResponse:
    """
    Эндпоинт, который принимает CSV-файл и возвращает его эвристики.
    ...
    has_suspicious_id_duplicates = bool(flags_all.get("has_suspicious_id_duplicates", False))
